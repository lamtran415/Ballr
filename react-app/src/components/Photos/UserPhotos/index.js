...
const UserPhotos = () => {
  ...
  const userPhotos = Object.values(
    useSelector((state) => state.photos.userPhotos)
  );

  return (
    <>
      {(!isLoaded || !userPhotos) && <LoadingPage />}

      {isLoaded && userPhotos && (
        <>
          {userPhotos.length === 0 && sessionUser?.id !== +userId && (
            <ErrorPage />
          )}
            ...
            <>
              <div className="user-header-container">
                <div className="user-header-width-div">
                  <img
                    src="https://combo.staticflickr.com/pw/images/buddyicon03.png#197638499@N08"
                    className="user-header-picture"
                    alt=""
                  />
                  <div className="user-header-information-container">
                    <div className="user-header-name-div">
                      {userPhotos.length !== 0
                        ? userPhotos[0]?.user?.first_name
                        : sessionUser.first_name}{" "}
                      {userPhotos.length !== 0
                        ? userPhotos[0]?.user?.last_name
                        : sessionUser.last_name}
                    </div>
                    <div className="user-email-photo-length">
                      <div>
                        {userPhotos.length !== 0
                          ? userPhotos[0]?.user?.email
                          : sessionUser.email}
                      </div>
                      <div className="user-header-photo-header">
                        {userPhotos?.length}{" "}
                        {userPhotos?.length ? "Photos" : "Photo"}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="photo-album-navlink-container">
                <div className="margin-div">
                  <NavLink
                    to={`/photos/users/${userId}`}
                    className="photo-album-navlink"
                  >
                    Photostream
                  </NavLink>
                  <NavLink
                    to={`/photos/users/${userId}/albums`}
                    className="photo-album-navlink"
                  >
                    Albums
                  </NavLink>
