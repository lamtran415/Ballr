...
const leastCommonFlavor = (candyBar) => {
    let valueHolder = Object.values(candyBar)[0];
    let keyHolder = "";

    for (let key in candyBar) {
        if (valueHolder >= candyBar[key]) {
            valueHolder = candyBar[key]
            keyHolder = key;
        }
    }

    if (Object.values(candyBar).length === 0) {
        return null
    }
    return keyHolder
}
